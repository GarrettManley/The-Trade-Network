...
public class NewUserPanel extends JPanel implements StringConstants{
	...
	public NewUserPanel(TheMainPanel mainpanel, DBConnection db) {
		...
		JLabel lblPassword = new JLabel("Password(must be 5-30 chars)");
		lblPassword.setBounds(234, 114, 188, 14);
		add(lblPassword);
		...
		//label for username
		JLabel lblUsername = new JLabel("Username(must be 5-30 chars)");
		lblUsername.setBounds(234, 69, 188, 14);
		add(lblUsername);
	...
	class NewUserPanelListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == goBack){
				...
				try {
					db.createUser(info[0], info[1], (info[2]), info[3], info[4], info[5], info[6]);
					JOptionPane.showMessageDialog(mainpanel,"New User Created!\nWelcome to the Trade Network");
					
					CardLayout cl = (CardLayout)(mainpanel.getLayout());
					cl.show(mainpanel, LOGIN);
					mainpanel.changeSize(450,300);
				} catch (SQLException e1) {
					createErrorMessage(e1.getErrorCode());
					e1.printStackTrace();
				}
		...
		//looks at error code sent when exception occurs
		//display correct error message dialog
		public void createErrorMessage(int errorcode){
			//check failed
			if(errorcode == 547){
				JOptionPane.showMessageDialog(mainpanel,"Error occured, field incorrect, please look over information");
			}
			//duplicate primary key
			else if(errorcode == 2627){
				JOptionPane.showMessageDialog(mainpanel, "Username already exists");
			}
		}
